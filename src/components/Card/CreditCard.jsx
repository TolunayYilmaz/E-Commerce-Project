...
export default function CreditCard({cardKey, item }) {
  ...
  const handleLinkClick = (e) => {
    e.preventDefault();
    setIsModalOpen(true);
    setFormHead("Update Card");

    if(formHead==="Update Card"){
      const fields = ["name_on_card", "card_no", "expire_month", "expire_year"];
  ...
  const onSubmit = (data) => {
    ...
    dispatch(updateCardItem({ ...data, id: item.id }));
  ...
  return (
    <div key={cardKey}>
      <div className="mx-2 flex justify-between">
        <div className="flex gap-1">
         <input
            ...
            name="creditCard"
            value={item.id}
            defaultChecked={item.id === cardList[0].id}
            onChange={()=>dispatch(setPayment(item))}
            className="bg-red-50"
          />
        ...
        <div className="flex">
          <Trash2
            ...
            size={16}
            onClick={() => dispatch(deleteCardItem(item?.id))}
          />
          <a
            className="text-black text-xs hover:cursor-pointer"
      ...
      <div className=" relative h-[120px] w-[200px] rounded-xl bg-gradient-to-r from-gray-500 to-gray-400 text-white shadow-2xl transition-transform hover:scale-110">
        ...
        <div className="absolute bottom-2 w-full px-3">
          <div className="text-[9px] font-light leading-tight text-white space-y-[2px]">
            <p>
              Member Since: {item.expire_month}/{item.expire_year}
            </p>
            <p>Status: Active</p>
          </div>
        </div>
